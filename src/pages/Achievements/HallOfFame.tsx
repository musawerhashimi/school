...
import { hallOfFameData } from "../../data/halloffame";
...
export default function HallOfFame() {
  ...
  const availableYears = useMemo(() => {
    const years = hallOfFameData.map((honoree) => honoree.year);
    return ["all", ...Array.from(new Set(years)).sort((a, b) => b - a)];
  }, []);

  // Filter and search logic
  const filteredHonorees = useMemo(() => {
    return hallOfFameData.filter((honoree) => {
          ...
          {filteredHonorees.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredHonorees.map((honoree) => (
                <HonoreeCard key={honoree.id} honoree={honoree} lang={lang} />
              ))}
            </div>
          ) : (
